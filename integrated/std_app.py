...
def dashboard():
    ...
    applications_rows = conn.execute(
...
def jobs_page():
    ...
    applications_rows = conn.execute(
    "SELECT job_id FROM applications WHERE student_email=?",
    (session.get('user'),)
).fetchall()
...
def post_job():
    ...
    if request.method == 'POST':
        ...
        cursor.execute("""
            INSERT INTO jobs
            (company, role, location, package, eligibility, skills, description)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (
            company,
            title,
