...
def init_db():
    ...
    # Ensure jobs table has an owner_email column to track which recruiter posted the job
    ...
    if request.method == 'POST':
        ...
        if not role or not name or not email or not password or not confirm:
            ...
            try:
                ...
                if role == "Student":
                    conn.execute(
                        "INSERT OR IGNORE INTO students (name, email, course, department, cgpa, year) VALUES (?, ?, ?, ?, ?, ?)",
                        (name, email, "", "",0.0, "")
                    )
                elif role == "Recruiter":
                    conn.execute(
                        "INSERT OR IGNORE INTO recruiters (name, email, company, status) VALUES (?, ?, ?, ?)",
                        (name, email, "","Active")
                    )

                    conn.execute(
                        "INSERT OR IGNORE INTO recruiter_profiles (email) VALUES (?)",
                        (email,)
                    )
                conn.commit()
                session['user'] = email
...
def profile():
    ...
    applications = conn.execute("""
        SELECT applications.*, jobs.role, jobs.company
        FROM applications
...
def update_profile():
    ...
    new_roll_no = request.form.get('roll_no') or student['roll_no']
    new_course = request.form.get('course') or student['course']
    new_department = request.form.get('department') or student['department']
    new_cgpa = request.form.get('cgpa') or student['cgpa']
    new_skills = request.form.get('skills') or student['skills']
    new_year = request.form.get('year') or student['year']

    conn.execute("""
        UPDATE students SET name=?, email=?, roll_no=?, course=?, department=?, cgpa=?, year=?, skills=? WHERE email=?
    """, (new_name, new_email, new_roll_no, new_course, new_department, new_cgpa, new_year, new_skills, current_email)
    )
...
def recruiter_dashboard():
    ...
    shortlisted = conn.execute("""
        SELECT * FROM applications
        WHERE status='Shortlisted' AND job_id IN (SELECT id FROM jobs WHERE owner_email = ?)
    """, (email,)).fetchall()

    selected = conn.execute("""
        SELECT * FROM applications
        WHERE status='Selected' AND job_id IN (SELECT id FROM jobs WHERE owner_email = ?)
    """, (email,)).fetchall()
...
def recruiter_profile():
    ...
    if not profile:
        conn.execute(
            "INSERT INTO recruiter_profiles (email) VALUES (?)",
            (email,)
        )
        conn.commit()
...
# POST JOB
@app.route('/rec_post_job', methods=['GET', 'POST'])
def post_job():
    ...
    if request.method == 'POST':
        ...
        posted_date = date.today().strftime("%d-%m-%Y")
    ...
    return render_template("rec_post_job.html")
...
def applicants():
    ...
    return render_template("rec_applicants.html", applicants=applications)
...
def update_application_status():
    ...
    conn = get_db_connection()
    # Only allow updating applications that belong to jobs posted by this recruiter
    conn.execute("""
        UPDATE applications
        SET status=?
        WHERE id=? AND job_id IN (SELECT id FROM jobs WHERE owner_email = ?)
    """, (status, app_id, email))

    conn.commit()
    conn.close()

    return redirect('/rec_applicants')
...
def recruiters():
    ...
    recruiters = conn.execute("""
        SELECT 
            ...
            recruiter_profiles.industry
        FROM recruiters
        LEFT JOIN recruiter_profiles
        ON recruiters.email = recruiter_profiles.email
    """).fetchall()
    conn.close()
...
def reports():
    ...
    recruiters_count = conn.execute("SELECT COUNT(*) FROM recruiters").fetchone()[0]
    jobs_count = conn.execute("SELECT COUNT(*) FROM jobs").fetchone()[0]
    applications_count = conn.execute("SELECT COUNT(*) FROM applications").fetchone()[0]
